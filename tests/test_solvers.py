...
class TestConjugateGradientSolver(unittest.TestCase):
    ...
    def test_entry(self):
        ...
        print '-------------------------------------------------------- entry'
        print 'cost.cost_function(X_estimated)', cost.cost_function(X_estimated)
        print 'cost.cost_function(X_true)', cost.cost_function(X_true)
        print '-------------------------------------------------------- '
    ...
    def test_blind(self):
        ...
        solver = ConjugateGradientSolver(cost.cost_function, self.data, self.rank, 3, verbosity=0)
        self.data.d = solver.recover()
        X_estimated = self.data.d['sample']['estimated_bias']
        X_true = self.data.d['sample']['true_bias']
